...
class TasksTree(object):
    ...
    def __init__(self, title=None, task_id=None, task_notes=None, task_status=None, task_due=None, links=None):
        self.title = title
    ...
    def add_subtask(self, title, task_id = None, parent_id = None,
            task_notes=None, task_status=None, task_due=None, links=None):
        """
        ...
        if parent_id is None:
            self.subtasks.append(
                TasksTree(title, task_id, task_notes, task_status, task_due, links))
        else:
            ...
            self.get_task_with_id(parent_id).add_subtask(title, task_id, None,
                    task_notes, task_status, task_due, links)
    ...
    def _lines(self, level):
        ...
        for subtask in self.subtasks:
            ...
            if subtask.notes is not None:
                ...
                res.append(notes)
            if subtask.links:
                res.append(":michel-orgmode-links:")
                for link in subtask.links:
                    # Escape link and description after `org-link-make-string`
                    # in Org mode.
                    # LINK is escaped after `org-link-escape` (this logic is
                    # equivalent but a little easier to follow).
                    l = re.sub(r'(\\+)($|[][])', r'\1\1\2', link[u'link'])
                    l = re.sub(r'([][])', r'\\\1', l)
                    # DESCRIPTION escaping
                    d = re.sub(r'\]\]', u'\]\N{ZERO WIDTH SPACE}\]', link[u'description'])
                    d = re.sub(r'\]$', u'\]\N{ZERO WIDTH SPACE}', d)
                    res.append('- {}: [[{}][{}]]'.format(link[u'type'], l, d))
                res.append(":END:")
